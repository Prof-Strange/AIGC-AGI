...
\begin{itemize}
    ...
    \item Example: If ``S'' denotes a private sensation, there's no way to confirm if today's ``S'' matches yesterday's without communal agreement.
\end{itemize}
...
In DID, an individual has multiple alters with fragmented communication, leading to inconsistent realities \cite{putnam1989}. Similarly:

\begin{itemize}
    \item Mathematicians specialize in niches, rarely mastering others' work holistically.
    \item Proofs span hundreds of pages, relying on prior results that few fully internalize.
    \item Without machine aid, verification is like alters sharing uncheckable memories: ``I feel this proof is correct,'' but no public criterion beyond intuition.
\end{itemize}

Thus, the meta-person's language is private--internal to its dissociated self--lacking sufficient external grounding.
...
One might argue that communal peer review provides public criteria. However, in complex noninterdisciplinary pure math, review is often partial; trust in authority supplants verification.
...
\section{Conclusion}
Extending Wittgenstein's PLA, we conclude that without machine-aided proofs, modern noninterdisciplinary pure mathematics is an ungrounded private language of a DID-afflicted meta-person. This highlights the epistemological role of computation in sustaining abstract discourse within this subset.
